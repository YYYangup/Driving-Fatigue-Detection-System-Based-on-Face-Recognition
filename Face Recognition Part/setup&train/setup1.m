...
options.datasetName ='lfw';  %%'lfw_color'; %'lfpw'; %'helen';  or 'w300'
% load('pts');
% options.pts = pts;
fs=[]; mean_shape=[];
load('fs');load('ind'); load('mean_shape');
pl=1:size(fs,1);
sind=setdiff(pl,ind);
options.lfwind = sind;
options.sts = fs(:,[18:66 84:end]);
options.mean_shape = mean_shape;
