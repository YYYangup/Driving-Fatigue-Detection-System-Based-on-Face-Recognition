function [Data] = load_all_data3 ( dbpath_img, options )
slash = options.slash;

%% ����lfw database
if strcmp(options.datasetName,'lfw49') == 1 || strcmp(options.datasetName,'lfw66') == 1
   shape = options.pts;
   nimgs = size(shape,1);
else
   lfwind = options.lfwind;
   nimgs  = length(lfwind);
   shape = options.sts;  
   shape = shape(options.lfwind,:);
end


%shape = options.pts;
%nimgs = size(options.pts,1);
...
for iimgs = 1 : nimgs
    ...
    %% load images
    %% lfw
    img = im2uint8(imread([dbpath_img slash num2str( iimgs ) '.jpg']));
    ...
    %% load shape
    Data{iimgs}.shape_gt = reshape(shape(iimgs,:),size(shape,2)/2,2);
